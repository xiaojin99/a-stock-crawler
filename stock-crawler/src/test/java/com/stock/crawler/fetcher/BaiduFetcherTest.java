...
/**
 * 百度热搜获取器集成测试（需要网络，运行: mvn test -Dintegration=true）
 */
@Tag("integration")
@DisplayName("百度热搜获取器集成测试")
class BaiduFetcherTest {

    private BaiduFetcher baiduFetcher;

    @BeforeEach
    void setUp() {
        baiduFetcher = new BaiduFetcher();
    }

    @Test
    @Timeout(value = 30, unit = TimeUnit.SECONDS)
    @DisplayName("测试获取百度热搜列表")
    void testFetch() throws IOException {
        List<HotItem> items = baiduFetcher.fetch();
    ...
    void testPlatformInfo() {
        assertEquals("baidu", baiduFetcher.platform());
        assertEquals("百度热搜", baiduFetcher.platformCN());

        System.out.println("=== 平台信息 ===");
        System.out.println("平台ID: " + baiduFetcher.platform());
        System.out.println("平台名称: " + baiduFetcher.platformCN());
    }
    ...
    void testDataIntegrity() throws IOException {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            // ID 格式验证
            assertTrue(item.getId().startsWith("baidu_"), "ID 应以 'baidu_' 开头");
        }
